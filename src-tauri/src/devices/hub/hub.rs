...
use super::command_handler::HubCommand;
...
pub struct Hub {
    ...
    commands: Option<mpsc::Receiver<HubCommand>>,
...
impl Hub {
    ...
    pub async fn exec(&self, input: &str) -> String {
        ...
        let _ = self.command_tx.send(HubCommand::Exec { input: input.to_string(), reply: tx }).await;
        rx.await.unwrap_or_else(|_| "device not running".into())
    }

    pub fn start(&mut self) -> tokio::task::JoinHandle<()> {
        let mut commands = self.commands.take().expect("Hub already started");
...
impl Device for Hub {
    ...
    async fn on_command(&mut self, cmd: HubCommand) {
